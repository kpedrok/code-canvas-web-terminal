...
export const useFileStore = create<FileState>((set, get) => ({
    ...
    try {
      ...
      set((state) => ({
        files: state.files.map(f => 
          f.id === newFileId ? { ...f, id: savedFile.id } : f
        ),
      }));
    } catch (error) {
  ...
  deleteFile: async (id) => {
    ...
    const activeFileId = get().activeFileId === id 
  ...
  fetchProjectFiles: async (projectId) => {
    try {
      ...
      const files = projectFiles.map((apiFile: any) => ({
        id: apiFile.id,
        name: apiFile.name,
        language: apiFile.name.endsWith('.py') ? 'python' : 
                 apiFile.name.endsWith('.js') ? 'javascript' :
                 apiFile.name.endsWith('.ts') ? 'typescript' :
                 apiFile.name.endsWith('.html') ? 'html' :
                 apiFile.name.endsWith('.css') ? 'css' : 'plaintext',
